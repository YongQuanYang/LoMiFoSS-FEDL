...
work_dir="./work_dirs/imagenet_retrain_train0.95_val_${model}_swa_finetune_swalr${swa_lr}_constant_lr_2gpu_ensemble_index${ensemble_index}_greedy_step_emb"
greed_ensembling=1
mkdir -p ${work_dir}
...
CUDA_VISIBLE_DEVICES=0,1 python  experiments/imagenet/run_swag_imagenet.py \
    ...
    --batch_size=256 \
    --pretrained \
    --greed_ensembling=${greed_ensembling} \
